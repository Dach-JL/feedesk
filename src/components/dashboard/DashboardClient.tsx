      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {cards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className="bg-card rounded-2xl border border-border p-6 relative overflow-hidden group hover:shadow-md transition-all duration-300"
            >
              {/* Subtle accent bar */}
              ...
              <div className="text-sm font-medium text-muted-foreground mb-1">{card.label}</div>
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-destructive/10 flex items-center justify-center">
              <AlertCircle className="w-[18px] h-[18px] text-destructive" />
            </div>
            <div>
              <h3 className="text-base font-bold text-foreground">Unpaid Students Report</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Accounts requiring immediate attention</p>
            </div>
          </div>
          <Link 
            href="/dashboard/payments" 
            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-primary bg-primary/10 rounded-lg hover:bg-primary/20 transition-colors"
          >
            Process Payments
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Class</th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Owed</th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Paid</th>
              ...
              <tbody>
                {metrics.unpaidStudents.map(student => (
                  <tr key={student.id} className="border-b border-border/40 hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right text-muted-foreground tabular-nums">${student.totalOwed.toFixed(2)}</td>
                    <td className="px-6 py-4 text-right text-emerald-600 tabular-nums">${student.totalPaid.toFixed(2)}</td>
                    <td className="px-6 py-4 text-right font-bold text-destructive tabular-nums">${student.outstandingDues.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
